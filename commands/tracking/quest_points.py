...
REQUIRED_ROLES = (
    ...
    1457500571807383622, # Quest Master
)

# Badge thresholds (quest points : badge name)
BADGE_TIERS = [
    (350, "[Name] Badge"),
    (225, "Onyx"),
    (150, "Diamond"),
...
def get_badge_for_points(points: int) -> str:
    """Return the badge name corresponding to quest points."""
    current_badge = "No Badge"
...
def init_database():
    """Initialize the quest progress database"""
...
def setup(bot, has_required_role, config):
    ...
    @bot.tree.command(
        name="quest_points",
        description="Add or remove quest points for a player"
    )
    ...
    async def quest(interaction: discord.Interaction, player: discord.Member, points: int, esi_points: int, reason: str = ""):
        ...
        if points == 0:
            invalid_points = discord.Embed(
                title="Invalid Input",
                description="Quest points cannot be 0. Use positive or negative values.",
                color=0xFF0000,
                timestamp=datetime.utcnow()
            )
            await interaction.followup.send(embed=invalid_points)
        ...
        def db_operation():
            ...
            try:
                ...
                c.execute("SELECT points, badge FROM quest_progress WHERE player=?", (player_uuid,))
                row = c.fetchone()
                ...
                # Save new progress
                c.execute("""
                    INSERT INTO quest_progress (player, points, badge, last_updated)
                    VALUES (?, ?, ?, ?)
        ...
        description = f"**{player.mention}** (`{player_username}`) now has **{result['new_points']}** quest points."
        ...
        if result["badge_changed"] and result["new_points"] > result["current_points"]:
            ...
            description += f"\n\n**Badge Lost:** {player.mention} has been downgraded to **{result['new_badge']}**."
        ...
        esi.save_points(resolved, esi_points, f"Quest: {reason}" or "Quest points command")
